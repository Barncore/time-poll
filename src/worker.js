// src/worker.js
// Cloudflare Worker — API routes + static assets from public/ via Workers Assets.
// Data stored in KV (POLL_DATA binding) instead of data.json file.
...
async function handleAPI(request, env, path) {
  ...
  // Admin routes — all require password
  ...
  if (path === '/api/admin/data')   return json(data);
...
export default {
  async fetch(request, env) {
    ...
    // API requests → handled in JS
    if (path.startsWith('/api/')) {
      return handleAPI(request, env, path);
    }

    // Everything else → serve from public/ via Workers Assets
